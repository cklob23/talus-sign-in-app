...
export async function POST(request: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  // Visitor types are global, so only existence is checked. The NDA itself is
  // still resolved against this token's location below, so a token cannot pull
  // another site's agreement.
